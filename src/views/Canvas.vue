<template>
  <div class="wrap">
    ...
    <Alert ref="autoOpenSuccess" v-slot="slotProps" :boldText="'성공적으로 </br> 제출되었어요 :)'" :text="'결과를 확인해보세요'"
           :buttonText="'내 스테이지 확인하러 가기'"/>
...
export default {
  ...
  mounted () {
    ...
    if (this.page === '' || this.page === 'diagnose') {
      setTimeout(function () {
  ...
  methods: {
    ...
    exportPNG (e) {
      ...
      WILL.getImageCanvas().toBlob(function (blob) {
        let file = new File([blob], 'my_image.png', {
          type: 'image/png',
