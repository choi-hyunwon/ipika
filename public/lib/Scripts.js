...
var tools = {
  ...
  disableOverscroll: function () {
    ...
    document.addEventListener('touchstart', function (e) {
      deltaX = e.changedTouches[0].pageX
    }, { passive: false })

    document.addEventListener('touchmove', function (e) {
      if (deltaX != e.changedTouches[0].pageX) {
        e.preventDefault()
      }
    }, { passive: false })
  },
...
var movesCounter = {
  init: function (custom) {
    ...
    document.addEventListener('mousedown', function (e) {
      movesCounter.start(e)
    })
    if (!custom) {
      document.addEventListener('mousemove', function (e) {
        movesCounter.increment(e)
      })
    }
...
var dropDown = {
  init: function () {
    $('.DropDown .Paper').on('mousedown', function () {
      dropDown.selectPaper(this)
    })
    $('.DropDown .Color').on('mousedown', function () {
      dropDown.selectColor(this)
    })
  ...
  show: function (name) {
    ...
    var left = button.offsetLeft - $('.DropDown.' + name).width() / 2 + button.offsetWidth / 2 - 7
    $('.DropDown.' + name).show(500).css('left', left + 'px')
  },

  hide: function (name) {
    $('.DropDown.' + name).hide(500)
  },

  toggle: function (name) {
    if ($('.DropDown.' + name).css('display') == 'none') {
      this.show(name)
    } else {
      this.hide(name)
    }
...
var layout = {
  init: function () {
    ...
    if (this.navType == 'DEMO') {
      this.initDemoNavigation()
    } else if (window.location.pathname === '/canvas') menu.init()
